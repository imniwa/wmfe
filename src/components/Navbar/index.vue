<script>
import {
    useAuthStore
} from "@/stores/auth.store.js"
export default {
    name: 'Navbar',
    data(){
        return {
            authStore: useAuthStore(),
        }
    },
    methods: {
        handleShowMenu() {
            this.$refs.logoutmenu.classList?.toggle('hidden');
        },
        handleLogout() {
            this.authStore.logout();
            this.$router.push({
                name: 'login'
            });
        }
    }
}
</script>
<template>
    <nav class="bg-primary w-full">
        <div class="container mx-auto flex place-content-between text-white py-4 items-center">
            <button type="button" @click="this.$router.push({
                name: 'home'
            })">
                <LogoPrimary />
            </button>
            <div class="relative">
                <div class="flex gap-2 items-center rounded-md px-4 py-2 hover:bg-white/30 cursor-pointer"
                    @click="handleShowMenu">
                    <p>{{ authStore.user?.name || 'Guest' }}</p>
                    <svg width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="40" height="40" rx="20" fill="url(#pattern0_3641_157)" />
                        <rect x="28" y="28" width="13" height="13" rx="6.5" fill="#36B37E" />
                        <rect x="28" y="28" width="13" height="13" rx="6.5" stroke="white" stroke-width="2" />
                        <defs>
                            <pattern id="pattern0_3641_157" patternContentUnits="objectBoundingBox" width="1"
                                height="1">
                                <use xlink:href="#image0_3641_157"
                                    transform="translate(-0.00260417) scale(0.00520833)" />
                            </pattern>
                            <image id="image0_3641_157" width="193" height="192"
                                xlink:href="data:image/png;base64," />
                        </defs>
                    </svg>
                </div>
                <div class="absolute hidden bottom-[-40px] bg-white left-0 right-0 border rounded-md px-4 py-2"
                    ref="logoutmenu" @click="handleLogout">
                    <button type="button" class="text-red-500 w-full text-left">
                        Logout
                    </button>
                </div>
            </div>
        </div>
    </nav>
</template>